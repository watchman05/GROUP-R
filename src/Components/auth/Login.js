...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const isLogggedIn = await signInWithEmailAndPassword(auth, formData.email, formData.password);
      if(isLogggedIn && isLogggedIn.user){
        const username = localStorage.getItem("username")

        localStorage.setItem("isLoggedIn", "true");
        successNotify(`Welcome back, ${username}`)
      }
      setTimeout(() => {
        navigate("/notes")  
      }, 3000)
              ...
              <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                  ...
                  <form className="space-y-4 md:space-y-6">
                      <div>
                          ...
                          <input onChange={handleChange} value={formData.email} type="email" name="email" id="email" className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                      ...
                      <div className="flex items-center justify-between">
                          ...
                          <Link to="/auth/forgotpassword" className="text-sm font-medium text-primary-600 hover:underline dark:text-primary-500">Forgot password?</Link>
                      </div>
